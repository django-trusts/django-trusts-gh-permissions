...
Obtain the configured handle before either helper runs so invalid
configuration fails before partial mutation. Exact-duplicate
registration still fails closed without mutating stored records.
Re-entry to `ready()` is keyed on the handle, not `.registry`. Do not
call `.registry.register` from production donation. `handle.registry`
remains temporarily so compiler tests can inspect normalized records
and so object/list evaluation can keep `registry.has_permission` /
`.authorized()`.
