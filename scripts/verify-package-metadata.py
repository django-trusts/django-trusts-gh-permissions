...
def _check_license(notice: str, origin: str) -> None:
    if 'Copyright (c) 2026, BeeDesk, Inc.' not in notice:
        raise SystemExit('%s LICENSE notice is not BeeDesk 2026' % origin)
    if 'and contributors' in notice.split('THIS SOFTWARE')[0]:
        raise SystemExit('%s LICENSE still names contributors on the copyright line' % origin)
...
def _check_wheel(wheel: Path) -> None:
    with zipfile.ZipFile(wheel) as zf:
        ...
        _check_license(zf.read(license_hits[0]).decode(), wheel.name)
...
def _check_sdist(sdist: Path) -> None:
    with tarfile.open(sdist, 'r:gz') as tf:
        ...
        _check_license(tf.extractfile(license_name).read().decode(), sdist.name)
        ...
        if not any(name.endswith('/DEV.md') for name in names):
