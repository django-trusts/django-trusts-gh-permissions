...
class GhNamingTest(SimpleTestCase):
    def test_package_source_uses_gh_not_full_external_name(self):
        ...
        import gh_permissions.views
        for module in (
            gh_permissions, gh_permissions.admin, gh_permissions.models,
            gh_permissions.policy, gh_permissions.views,
        ):
    ...
    def test_adapter_names_are_gh_not_zero(self):
        self.assertEqual(TEAM_ADAPTER, 'team')
        self.assertEqual(DIRECT_ADAPTER, 'direct')
        self.assertNotEqual(TEAM_ADAPTER, 'group')
        self.assertNotIn(TEAM_ADAPTER, FORBIDDEN_PUBLIC_RELATIONS)
