"""GH-only C2 topology: kernel label, no Zero, inert trusts.models shim."""
...
class KernelTopologyTest(SimpleTestCase):
    ...
    def test_kernel_label_is_trusts_core(self):
    ...
    def test_repository_uses_authorized_manager(self):
        ...
        self.assertTrue(issubclass(Repository.objects._queryset_class, AuthorizedQuerySet))
        self.assertFalse(hasattr(Repository.objects, 'permitted'))
        self.assertFalse(hasattr(Repository.objects, 'get_permission'))
        self.assertFalse(hasattr(Operation.objects, 'authorized'))
        self.assertFalse(hasattr(Operation.objects, 'permitted'))
        self.assertFalse(hasattr(Operation.objects, 'get_permission'))


class KernelMigrationLoaderTest(TestCase):
    def test_no_trusts_schema_or_migrations(self):
...
class ZeroAbsentTest(SimpleTestCase):
    def test_trusts_zero_stays_out_of_sys_modules(self):
        self.assertNotIn('trusts.zero', sys.modules)
